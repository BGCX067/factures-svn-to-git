...
public class PayableRestClient {
	...
	private Base client;
//	private String endPoint = "http://127.0.0.1:8081/rest"; 
	...
	@Inject
	public PayableRestClient(EventBus eventBus){
		
		this.eventBus = eventBus;
		client = new Base(getEndPoint());
		this.eventBus.addHandlerToSource(PayableAddedEvent.TYPE, Payable.Type.FIX_COST, new PayableAddedHandler(){
			@Override
	...
	public void updateFixCost(FixCost factura, RequestCallback callback) throws RequestException{
		client.put(factura.serialize(),factura.getId(), "fixcost", callback);
	}
	...
	public void delete(final RequestCallback callback, String ressourceName, int id){
		try {
			client.delete(id, ressourceName, callback);
	...
	private String getEndPoint() {
		System.out.println("using " + GWT.getHostPageBaseURL() + "rest");
		return GWT.getHostPageBaseURL() + "rest";
	}
}
